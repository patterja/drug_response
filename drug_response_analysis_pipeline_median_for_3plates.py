...
import csv
import argparse
from drug_response.drug_plate_median import CellCountMedian
from drug_response.generate_input_for_gr50_tools import time0_median, generate_input
...
from drug_response.add_max_drug_does_to_GRmetrics import add_max_conc_to_GRm

#arguments parse
parser = argparse.ArgumentParser(description='')
parser.add_argument('--d', help='input the directory of raw data')
parser.add_argument('--w', help='input the work directory')
parser.add_argument('--f1', help='input the file names of raw data from plate 1')
parser.add_argument('--f2', help='input the file names of raw data from plate 2')
parser.add_argument('--f3', help='input the file names of raw data from plate 3')
parser.add_argument('--s', help='input the sample information file')
parser.add_argument('--l1', help='input the layout of palte 1')
parser.add_argument('--l2', help='input the layout of palte 2')
parser.add_argument('--l3', help='input the layout of palte 3')
...
os.system("mkdir -p %s %s %s %s %s %s %s %s" % (dir_var, dir_layout_heatmap, dir_CellCount, dir_GR, dir_cell_line_plot, dir_drug_plot, dir_stat_plot, dir_serum_plot))

#read in the files of plate 1 one by one
with open (args.f1, "r") as tsvfile:
    h_file=csv.reader(tsvfile, delimiter="\t")
    for lst_row in h_file:
        s_file_name=lst_row[0]
        s_file_path=os.path.join(args.d, s_file_name)
        #calculate the median value of cell count and write down
        df_median=CellCountMedian(s_file_path, args.l1)
        ...
        os.system('python3 /Users/zhanho/Project/SMMART/scripts/heatmap_for_384_matrix.py --i %s --l %s --h %s' % (s_file_path, args.l1, heatmap_file))
        #calculate the data variance
        (d_mean, d_cv,i_remove_num, plateID)=CellCountMeanFilter(s_file_path, args.l1)
...
#read in the files of plate 2 one by one
with open (args.f2, "r") as tsvfile:
    h_file=csv.reader(tsvfile, delimiter="\t")
    for lst_row in h_file:
        s_file_name=lst_row[0]
        s_file_path=os.path.join(args.d, s_file_name)
        #calculate the median value of cell count and write down
        df_median=CellCountMedian(s_file_path, args.l2)
        s_plateID=re.search("(\w+).txt", s_file_name)
        s_out_median=os.path.join(dir_CellCount, s_plateID.group(1) + "_CellCount_median.txt")
        df_median.to_csv(s_out_median, sep="\t")
        #plot the heatmap for the raw cell count
        heatmap_file=os.path.join(dir_layout_heatmap, s_plateID.group(1) + "_heatmap.pdf")
        os.system('python3 /Users/zhanho/Project/SMMART/scripts/heatmap_for_384_matrix.py --i %s --l %s --h %s' % (s_file_path, args.l2, heatmap_file))
        #calculate the data variance
        (d_mean, d_cv,i_remove_num, plateID)=CellCountMeanFilter(s_file_path, args.l2)
        #write out the variance
        s_out_var=os.path.join(dir_var, s_plateID.group(1) + "_CellCount_variance.txt")
        with open (s_out_var, "w") as tsvcv:
            h_cv=csv.writer(tsvcv, delimiter="\t")
            ...
            for key in d_cv.keys():
                lst_cv=d_cv[key].split("\t")
                h_cv.writerow(lst_cv)
#read in the files of plate 3 one by one
with open (args.f3, "r") as tsvfile:
    h_file=csv.reader(tsvfile, delimiter="\t")
    for lst_row in h_file:
        s_file_name=lst_row[0]
        s_file_path=os.path.join(args.d, s_file_name)
        #calculate the median value of cell count and write down
        df_median=CellCountMedian(s_file_path, args.l3)
        s_plateID=re.search("(\w+).txt", s_file_name)
        s_out_median=os.path.join(dir_CellCount, s_plateID.group(1) + "_CellCount_median.txt")
        df_median.to_csv(s_out_median, sep="\t")
        #plot the heatmap for the raw cell count
        heatmap_file=os.path.join(dir_layout_heatmap, s_plateID.group(1) + "_heatmap.pdf")
        os.system('python3 /Users/zhanho/Project/SMMART/scripts/heatmap_for_384_matrix.py --i %s --l %s --h %s' % (s_file_path, args.l3, heatmap_file))
        #calculate the data variance
        (d_mean, d_cv,i_remove_num, plateID)=CellCountMeanFilter(s_file_path, args.l3)
        #write out the variance
        s_out_var=os.path.join(dir_var, s_plateID.group(1) + "_CellCount_variance.txt")
        with open (s_out_var, "w") as tsvcv:
            h_cv=csv.writer(tsvcv, delimiter="\t")
...
#os.system('python3 /Users/zhanho/Project/SMMART/scripts/boxplot_for_GRmetrics.py --i %s --w %s' % (s_GRmetrics, dir_stat_plot))
#os.system('python3 /Users/zhanho/Project/SMMART/scripts/gr50_and_curve_count_barplot.py --i %s --p %f ---w %s' % (s_GRmetrics, "0.05", dir_stat_plot))
#
############GR50 heatmap plot############
...
#
############GR50 and drug concentration plot#############
#os.system('python3 /Users/zhanho/Project/SMMART/scripts/boxplot_for_GR50_and_serum_conc.py --i %s --c %s --s %s --w %s' % (s_GRmetrics, args.c, args.sc, dir_serum_plot))
